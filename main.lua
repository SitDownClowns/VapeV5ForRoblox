		...
		if (not teleportedServers) and (not shared.VapeIndependent) then
			...
			local teleportScript = [[
				...
				if shared.VapeDeveloper then
					...
					loadstring(game:HttpGet('https://raw.githubusercontent.com/SitDownClowns/VapeV5ForRoblox/'..readfile('vape/profiles/commit.txt')..'/loader.lua', true), 'loader')()
				end
			]]
			if shared.VapeDeveloper then
...
local XFunctions = loadstring(downloadFile('vape/libraries/XFunctions.lua'), 'XFunctions')()
...
if not shared.VapeIndependent then
	...
	loadstring(downloadFile('vape/games/modules.lua'), 'modules')()
	if isfile('vape/games/'..game.PlaceId..'.lua') then
		loadstring(readfile('vape/games/'..game.PlaceId..'.lua'), tostring(game.PlaceId))(...)
	else
		if not shared.VapeDeveloper then
			local suc, res = pcall(function()
				return game:HttpGet('https://raw.githubusercontent.com/SitDownClowns/VapeV5ForRoblox/'..readfile('vape/profiles/commit.txt')..'/games/'..game.PlaceId..'.lua', true)
			end)
